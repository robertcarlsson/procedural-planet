...
namespace Testing3D
{
    public class WaterI : DrawableGameComponent
    {
        ...
        public WaterI(Game game,string Environment, Camera cam) : base(game)
        {
        ...
        {
            effect = Game.Content.Load<Effect>("Shaders/Water");

            effect.Parameters["tEnvMap"].SetValue(Game.Content.Load<TextureCube>(EnvAsset));
            effect.Parameters["tNormalMap"].SetValue(Game.Content.Load<Texture2D>("Textures/waves2"));
            ...
            vb = new VertexBuffer(device, VertexMultitextured.VertexDeclaration, myWidth * myHeight,BufferUsage.WriteOnly);
           ...
           short[] terrainIndices = new short[(myWidth - 1) * (myHeight - 1) * 6];
            ...
            ib = new IndexBuffer(device, typeof(short), (myWidth - 1) * (myHeight - 1) * 6, BufferUsage.WriteOnly);
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            Matrix WV = World * camera.viewMatrix;
            Matrix viewI = Matrix.Invert(camera.viewMatrix);
        ...
        {
            effect.Parameters["fTime"].SetValue((float)gameTime.ElapsedGameTime.TotalSeconds);
